...
decoder = Decoder(len(output_vocab), embed_dim=512, hidden_dim=512, num_layers=4, rnn_type='LSTM',use_attention=False,encoder_output_dim=1024)
model = Seq2Seq(encoder, decoder, rnn_type='LSTM').to(DEVICE)
model.load_state_dict(torch.load("saved/seq2seq_model_vanilla.pt", map_location=DEVICE))
model.eval()
...
output_file = "prediction_vanilla.tsv"
